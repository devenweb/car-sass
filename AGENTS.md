...
- **Admin App**: Updated Settings dashboard to include a dedicated SMTP configuration section (Host, Port, User, Pass, Sender) for direct mail server management. Enhanced the Pricing Manager to explicitly indicate "Inactive" (Draft) vehicles in the sidebar for better fleet visibility.
